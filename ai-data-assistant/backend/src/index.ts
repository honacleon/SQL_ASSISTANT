...
// Resolver conflito de variáveis de ambiente (força uso do .env local)
...
// Load environment variables (override=true)
dotenv.config({ override: true });

// Log das API keys carregadas (apenas primeiros caracteres)
logger.info(`🔑 OpenAI Key: ${process.env.OPENAI_API_KEY?.substring(0, 20)}...`);
logger.info(`🔑 Anthropic Key: ${process.env.ANTHROPIC_API_KEY?.substring(0, 20)}...`);
